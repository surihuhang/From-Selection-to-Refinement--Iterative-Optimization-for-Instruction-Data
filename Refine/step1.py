...
# (API_BASE, MODEL_NAME 等已移至 __main__ 中)
...
def chat_with_llama(
    ...
    timeout, 
    max_retries, 
    system_prompt="You are a helpful assistant. Answer the following questions."
):
    """
    (函数逻辑保持不变)
    """
...
def process_item(
    ...
    item[round_tag_field_instruction] = item["instruction"]
    del item["instruction"]

    item[round_tag_field_input] = item["input"]
    del item["input"]

    instr = item.get(round_tag_field_instruction, "")
    inp = item.get(round_tag_field_input, "")
    ...
    item[round_tag_field_output_model] = response
...
def main(
    ...
    items_processed = 0
    # 
    update_interval = max(1, total_items // 100) # 每 1% 更新一次 ETR
    # --- 修改结束 ---
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Process data using a local LLM API.")
    
    parser.add_argument("--dataset_dir", type=str, required=True, help="Directory of the input dataset.")
    parser.add_argument("--base_dir", type=str, required=True, help="Base directory for output.")
    ...
    input_path = os.path.join(args.dataset_dir, args.Name + "_stage_2_low.json")
    output_path = os.path.join(args.base_dir, args.Name, "step1_output", "Low.jsonl")
    final_output_path = os.path.join(args.base_dir, args.Name, "step1_output", "Low.json")
